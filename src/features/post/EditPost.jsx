...
export default function CreatePost({ onSuccess, onClose, postObj }) {
  const [file, setFile] = useState(null);
  const [message, setMessage] = useState(postObj.message);
  ...
  const handleEditPost = async (postId) => {
    try {
      ...
      setLoading(true);
      await axios.patch(`/post/editPost/${postId}`, formData);
      onSuccess();
      onClose();
    } catch (err) {
      console.log(err);
  ...
  return (
    <div>
      ...
      <div
        ...
        onClick={() => handleEditPost(postObj.id)}
      >
        Submit Edit
      </div>
    </div>
